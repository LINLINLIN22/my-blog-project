...
![](select.png)
4. Quarto 會自動生出設定檔，先暫時不要動它，來試跑看看。試跑有2種方法:
- 第一種：在 Terminal cd 到目前 Project 的資料夾，使用 `quarto preview` 指令。這會另外開啟常用瀏覽器檢視。
...
quarto render
```
我的不知道為什麼會生成一個叫 `doc` 的資料夾，但 Github Page 只認 `docs`，所以生成後要修改檔名 !!!
