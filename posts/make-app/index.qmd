...
subtitle: "第一次用python做exe，不會的問AI"
author: "紙魚"
date: "2025-08-9"
categories: [python,toys]
image: "image.png"
## code cell
# format:
#   html:
...
這條程式需要放在主視窗 UI 剛建立的時候，也就是`app = ctk.CTk()`之後，不然會報錯。
...
> 如果不使用`resource_path()`會怎麼樣？
>
> 由於 PyInstaller  會把打包成一個 `.exe`。當執行這個 `.exe` 時，它會先解壓所有檔案到一個臨時目錄（通常是 %TEMP%\_MEIxxxxx），然後才開始執行程式。如果沒有`resource_path()`就會導致找不到檔案，因為它找到臨時目錄去了。

### 加入自定義圖示{#ico}

正常不做調整的情況下，視窗圖示為![](image4.png)，但我想換掉變成自己的。先找一個 `.png` 轉成 `.ico` 圖示，放進同一個專案資料夾。接下來要做的事跟前面的音檔類似 :
...
> 注意：
>
> 如果有[設定前面的圖示](#ico)，一樣需要用 `--add-data "XXX.ico;."`，不然它會找不到檔案。
...
涉及作業系統相關的知識，基礎用語請見[此來源](https://medium.com/erens-tech-book/%E7%90%86%E8%A7%A3-process-thread-94a40721b492)。 每當執行一個 `.exe`，作業系統就會開一個新的 process，由這個 process 執行你的程式，並從主執行緒 ( main tread )開始跑起。在程式中用 `threading.Thread()` 則會再額外開新的tread，也就是說每按下一個按鈕事件，就會啟動一個新的 tread。

這也讓目前的小程式有個問題，當過於頻繁地切換按鈕事件時程式容易當機，且容易有兩個按鈕 tread 同時在跑的狀況，是未來可以再精進的部分。
